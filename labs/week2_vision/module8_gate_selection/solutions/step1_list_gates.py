...
Week 2 · Module 8 — Step 1: List All Gates  (SOLUTION)
"""
...
def update(drone):
    ...
    if _timer >= HOVER_TIME:
        print(f"[Step 1] {len(gates)} gate(s) in view")
        for i, (col, area, _c) in enumerate(sorted(gates, key=lambda g: -g[1])):
            print(f"    gate {i}: col={col:.0f}  area={area:.0f}")
        _done = True
    return _done
